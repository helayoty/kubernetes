...
func TestWorkloadManager_AddPod(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:                  "adding a scheduled pod that was previously assumed",
			initPods:              []*v1.Pod{p2},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if inScheduled := info.ScheduledPods().Has(tt.podToAdd.UID); inScheduled != tt.expectInScheduledPods {
				t.Errorf("Unexpected ScheduledPods state, want: %v, got: %v", tt.expectInScheduledPods, inScheduled)
			}
		})
	}
}
