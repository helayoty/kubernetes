...
// The UpdatePod method is not needed because the binding is handled by scheduler event handlers
// as separate Pod/Delete and Pod/Add events. This way, AddPod handles the binding by
// removing the pod from assumedPods and adding it to scheduledPods.
...
// Pod is removed from the available and assumed pods sets for its corresponding pod group.
func (wm *workloadManager) DeletePod(pod *v1.Pod) {
	...
	if pod.Spec.NodeName == "" && info.scheduledPods.Has(pod.UID) {
		// The pod was bound, but the delete event doesn't reflect that.
		// This might happen because of scheduling queue implementation
