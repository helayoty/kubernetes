...
func (pl *GangScheduling) EventsToRegister(_ context.Context) ([]fwk.ClusterEventWithHint, error) {
	...
	return []fwk.ClusterEventWithHint{
		...
		// A Workload being added or updated could set or change a gang policy,
		// potentially making a waiting gang schedulable.
		{Event: fwk.ClusterEvent{Resource: fwk.Workload, ActionType: fwk.Add | fwk.Update}, QueueingHintFn: pl.isSchedulableAfterWorkloadChange},
	}, nil
}
...
func (pl *GangScheduling) isSchedulableAfterWorkloadChange(logger klog.Logger, pod *v1.Pod, oldObj, newObj interface{}) (fwk.QueueingHint, error) {
	originalWorkload, modifiedWorkload, err := util.As[*schedulingapi.Workload](oldObj, newObj)
	if err != nil {
		return fwk.Queue, err
	}

	if pod.Spec.Workload == nil || pod.Namespace != modifiedWorkload.Namespace || pod.Spec.Workload.Name != modifiedWorkload.Name {
		logger.V(5).Info("workload was added or updated but it doesn't match the target pod's workload",
			"pod", klog.KObj(pod), "modifiedWorkload", klog.KObj(modifiedWorkload))
		return fwk.QueueSkip, nil
	}

	if originalWorkload == nil {
		logger.V(5).Info("workload was added and it matches the target pod's workload, which may make the pod schedulable",
			"pod", klog.KObj(pod), "modifiedWorkload", klog.KObj(modifiedWorkload))
		return fwk.Queue, nil
	}

	originalPolicy, originalOk := podGroupPolicy(originalWorkload, pod.Spec.Workload.PodGroup)
	modifiedPolicy, modifiedOk := podGroupPolicy(modifiedWorkload, pod.Spec.Workload.PodGroup)
	if originalOk && !modifiedOk || !originalOk && modifiedOk {
		return fwk.Queue, fmt.Errorf("unexpected modification of workload pod groups")
	}
	if !modifiedOk {
		logger.V(5).Info("workload was updated, but it doesn't have any matching pod group for pod",
			"pod", klog.KObj(pod), "modifiedWorkload", klog.KObj(modifiedWorkload))
		return fwk.QueueSkip, nil
	}
	switch modifiedPolicy.Kind {
	case schedulingapi.PodGroupPolicyKindDefault:
		logger.V(5).Info("workload was updated and a new matching pod group has a Default policy, which may make the pod schedulable",
			"pod", klog.KObj(pod), "modifiedWorkload", klog.KObj(modifiedWorkload))
		return fwk.Queue, nil
	case schedulingapi.PodGroupPolicyKindGang:
		if originalPolicy.Gang.MinCount > modifiedPolicy.Gang.MinCount {
			logger.V(5).Info("workload was updated and a new matching pod group's minCount is lower than before, which may make the pod schedulable",
				"pod", klog.KObj(pod), "modifiedWorkload", klog.KObj(modifiedWorkload))
			return fwk.Queue, nil
		}
	}

	logger.V(5).Info("workload was updated, but its new policy is not making the pod schedulable",
		"pod", klog.KObj(pod), "modifiedWorkload", klog.KObj(modifiedWorkload))
	return fwk.QueueSkip, nil
}
...
func (pl *GangScheduling) PreEnqueue(ctx context.Context, pod *v1.Pod) *fwk.Status {
	if !pl.enabled || pod.Spec.Workload == nil {
		return nil
	}
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			...
			return fwk.NewStatus(fwk.UnschedulableAndUnresolvable, "waiting for pods's workload to appear in scheduling queue")
		}
	...
	if policy.Kind != schedulingapi.PodGroupPolicyKindGang {
		return nil
	}
