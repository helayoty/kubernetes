...
func Test_isSchedulableAfterWorkloadChange(t *testing.T) {
	tests := []struct {
		name                     string
		pod                      *v1.Pod
		oldWorkload, newWorkload *schedulingapi.Workload
		expectedHint             fwk.QueueingHint
	}{
		{
		...
		{
			name:         "update a workload with lower min count",
			pod:          st.MakePod().Name("p").Workload(&v1.WorkloadReference{Name: "w1", PodGroup: "pg"}).Obj(),
			oldWorkload:  st.MakeWorkload().Name("w1").PodGroup(st.MakePodGroup().Name("pg").MinCount(2).Obj()).Obj(),
			newWorkload:  st.MakeWorkload().Name("w1").PodGroup(st.MakePodGroup().Name("pg").MinCount(1).Obj()).Obj(),
			expectedHint: fwk.Queue,
		},
		{
			name:         "update a workload changing policy to default",
			pod:          st.MakePod().Name("p").Workload(&v1.WorkloadReference{Name: "w1", PodGroup: "pg"}).Obj(),
			oldWorkload:  st.MakeWorkload().Name("w1").PodGroup(st.MakePodGroup().Name("pg").MinCount(2).Obj()).Obj(),
			newWorkload:  st.MakeWorkload().Name("w1").PodGroup(st.MakePodGroup().Name("pg").DefaultPolicy().Obj()).Obj(),
			expectedHint: fwk.Queue,
		},
		{
			name:         "update a workload which doesn't match the pod's workload name",
			pod:          st.MakePod().Name("p").Workload(&v1.WorkloadReference{Name: "w2", PodGroup: "pg"}).Obj(),
			oldWorkload:  st.MakeWorkload().Name("w1").PodGroup(st.MakePodGroup().Name("pg").MinCount(2).Obj()).Obj(),
			newWorkload:  st.MakeWorkload().Name("w1").PodGroup(st.MakePodGroup().Name("pg").MinCount(1).Obj()).Obj(),
			expectedHint: fwk.QueueSkip,
		...
		{
			name:         "update a workload which doesn't match the pod's workload namespace",
			pod:          st.MakePod().Namespace("ns1").Name("p").Workload(&v1.WorkloadReference{Name: "w1", PodGroup: "pg"}).Obj(),
			oldWorkload:  st.MakeWorkload().Namespace("ns2").Name("w1").PodGroup(st.MakePodGroup().Name("pg").MinCount(2).Obj()).Obj(),
			newWorkload:  st.MakeWorkload().Namespace("ns2").Name("w1").PodGroup(st.MakePodGroup().Name("pg").MinCount(1).Obj()).Obj(),
			expectedHint: fwk.QueueSkip,
		},
		{
			name:         "update a workload which doesn't match the pod's pod group",
			pod:          st.MakePod().Name("p").Workload(&v1.WorkloadReference{Name: "w1", PodGroup: "pg"}).Obj(),
			oldWorkload:  st.MakeWorkload().Name("w1").PodGroup(st.MakePodGroup().Name("pg2").MinCount(2).Obj()).Obj(),
			newWorkload:  st.MakeWorkload().Name("w1").PodGroup(st.MakePodGroup().Name("pg2").MinCount(1).Obj()).Obj(),
			expectedHint: fwk.QueueSkip,
		},
		{
			name:         "update a workload with higher min count",
			pod:          st.MakePod().Name("p").Workload(&v1.WorkloadReference{Name: "w1", PodGroup: "pg"}).Obj(),
			oldWorkload:  st.MakeWorkload().Name("w1").PodGroup(st.MakePodGroup().Name("pg").MinCount(1).Obj()).Obj(),
			newWorkload:  st.MakeWorkload().Name("w1").PodGroup(st.MakePodGroup().Name("pg").MinCount(2).Obj()).Obj(),
			expectedHint: fwk.QueueSkip,
		},
		{
			name:         "update a workload with equal min count",
			pod:          st.MakePod().Name("p").Workload(&v1.WorkloadReference{Name: "w1", PodGroup: "pg"}).Obj(),
			oldWorkload:  st.MakeWorkload().Name("w1").PodGroup(st.MakePodGroup().Name("pg").MinCount(1).Obj()).Obj(),
			newWorkload:  st.MakeWorkload().Name("w1").PodGroup(st.MakePodGroup().Name("pg").MinCount(1).Obj()).PodGroup(st.MakePodGroup().Name("pg2").MinCount(2).Obj()).Obj(),
			expectedHint: fwk.QueueSkip,
		},
	}
	for _, tc := range tests {
...
func TestGangSchedulingFlow(t *testing.T) {
	...
	defaultPolicyWorkload := st.MakeWorkload().Namespace("ns1").Name("default-wl").
		PodGroup(st.MakePodGroup().Name("pg1").DefaultPolicy().Obj()).Obj()
	...
	defaultPolicyPod := st.MakePod().Namespace("ns1").Name("default-pod").UID("default-pod").
		Workload(&v1.WorkloadReference{Name: "default-wl", PodGroup: "pg1"}).Obj()
	...
	tests := []struct {
	...
	}{
		{
			...
			pod:                  nonGangPod,
			initialWorkloads:     []*schedulingapi.Workload{workload, defaultPolicyWorkload},
			wantPreEnqueueStatus: nil,
			wantPermitStatus:     nil,
		},
		{
			name:                 "default policy pod succeeds immediately",
			pod:                  defaultPolicyPod,
			initialWorkloads:     []*schedulingapi.Workload{workload, defaultPolicyWorkload},
			wantPreEnqueueStatus: nil,
			wantPermitStatus:     nil,
		},
		{
			...
			initialWorkloads:     []*schedulingapi.Workload{},
			wantPreEnqueueStatus: fwk.NewStatus(fwk.UnschedulableAndUnresolvable, "waiting for pods's workload to appear in scheduling queue"),
		},
		{
