...
var CoreResourceEnqueueTestCases = []*CoreResourceEnqueueTestCase{
	...
	{
		Name:          "Pod rejected by the GangScheduling plugin is requeued when a matching workload's pod group is updated to default policy",
		EnablePlugins: []string{names.GangScheduling},
		InitialNodes: []*v1.Node{
			st.MakeNode().Name("fake-node1").Obj(),
		},
		...
		TriggerFn: func(testCtx *testutils.TestContext) (map[fwk.ClusterEvent]uint64, error) {
			workload := st.MakeWorkload().Name("w1").PodGroup(st.MakePodGroup().Name("pg1").DefaultPolicy().Obj()).Obj()
			if _, err := testCtx.ClientSet.SchedulingV1alpha1().Workloads(testCtx.NS.Name).Update(testCtx.Ctx, workload, metav1.UpdateOptions{}); err != nil {
				return nil, fmt.Errorf("failed to update Workload %q: %w", workload.Name, err)
			}
			return map[fwk.ClusterEvent]uint64{{Resource: fwk.Workload, ActionType: fwk.Update}: 1}, nil
		},
		WaitingInUnschedulable:    true,
		WantRequeuedPods:          sets.New("pod1"),
		EnableSchedulingQueueHint: sets.New(true),
	...
	{
		Name:          "Pod rejected by the GangScheduling plugin is requeued when a matching workload's pod group gang policy is updated to lower minCount",
		EnablePlugins: []string{names.GangScheduling},
		InitialNodes: []*v1.Node{
			st.MakeNode().Name("fake-node1").Obj(),
		},
